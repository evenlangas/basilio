...
export default function ListSettingsPage({ params }: { params: { id: string } }) {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    setInviting(true);
    try {
      const response = await fetch(`/api/shopping-lists/${params.id}/invite`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        setInviteEmail('');
        loadList(); // Refresh to show new invite
      } else {
