...
export default function MyFollowersPage() {
  ...
  const loadFollowers = async () => {
    try {
      const response = await fetch(`/api/user/${session?.user?.id}/followers`);
      if (response.ok) {
        ...
        {loading ? (
          ...
          <div className="text-center py-8 sm:py-12 px-4">
            ...
            <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300 mb-4 sm:mb-6">
              Share your recipes and creations to attract followers!
            </p>
            <Link
              href="/explore"
              ...
              <div key={follower._id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-3 sm:p-4">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center gap-1.5 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg text-xs sm:text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0 min-w-[90px] sm:min-w-[110px] ${
